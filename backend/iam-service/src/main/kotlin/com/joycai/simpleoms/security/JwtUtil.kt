...
class JwtUtil(
    ...
    private fun parseClaims(token: String): Claims {
        // Extract kid from JWT header to look up the exact key directly
        val kidFromHeader: String? = runCatching {
            val headerJson = String(Base64.getUrlDecoder().decode(token.substringBefore('.')))
            Regex("\"kid\"\\s*:\\s*\"([^\"]+)\"").find(headerJson)?.groupValues?.get(1)
        }.getOrNull()

        // Fast path: look up by kid
        if (kidFromHeader != null) {
            val key = signingKeyRepository.findByKid(kidFromHeader)
            if (key != null) {
                return Jwts.parser()
        ...
        // Fallback: try all keys (supports tokens signed before a key rotation)
