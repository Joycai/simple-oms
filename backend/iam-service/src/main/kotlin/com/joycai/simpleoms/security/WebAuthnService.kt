...
class WebAuthnService(
    ...
    fun startLogin(username: String?): Map<String, Any> {
        ...
        // Use a per-request sessionId to avoid key collisions for concurrent anonymous logins
        val sessionId = UUID.randomUUID().toString()
        redisSet("login:$sessionId", assertion.toJson())
        ...
        @Suppress("UNCHECKED_CAST")
        val options = (full["publicKeyCredentialRequestOptions"] as? Map<String, Any>)
            ?: (full["publicKey"] as? Map<String, Any>)
            ?: full
        return options + mapOf("sessionId" to sessionId)
    }

    fun finishLogin(sessionId: String?, username: String?, credential: Map<String, Any>): String? {
        val key = "login:${sessionId ?: return null}"
