...
class AuthController(
    ...
    fun verifyAndEnableOtp(@Valid @RequestBody request: TotpVerifyRequest, @AuthenticationPrincipal username: String): ResponseEntity<Map<String, Any>> {
        ...
        user.recoveryCodes = codes.map { passwordEncoder.encode(it) }.joinToString("|")
        userRepository.save(user)
    ...
    fun getRecoveryCodes(@AuthenticationPrincipal username: String): ResponseEntity<Map<String, Any>> {
        // Recovery codes are hashed; we cannot reveal them after initial setup
        val hasAny = userRepository.findByUsername(username)!!.recoveryCodes != null
        return ResponseEntity.ok(mapOf("hasRecoveryCodes" to hasAny))
    }
    ...
    fun webauthnLoginFinish(@RequestBody body: Map<String, Any>): ResponseEntity<Map<String, Any>> {
        ...
        @Suppress("UNCHECKED_CAST")
        val credential = body["credential"] as? Map<String, Any> ?: return ResponseEntity.badRequest().body(mapOf("message" to "Missing credential"))
        val resolved = webAuthn.finishLogin(sessionId, username, credential)
    ...
    private fun otpVerify(user: User, code: String): Boolean {
        ...
        // Try recovery codes (stored as BCrypt hashes separated by '|')
        val hashes = user.recoveryCodes?.split("|")?.toMutableList() ?: return false
        val matchIndex = hashes.indexOfFirst { passwordEncoder.matches(code.trim(), it) }
        if (matchIndex == -1) return false
        hashes.removeAt(matchIndex)
        user.recoveryCodes = if (hashes.isEmpty()) null else hashes.joinToString("|")
        userRepository.save(user)
        return true
    }
