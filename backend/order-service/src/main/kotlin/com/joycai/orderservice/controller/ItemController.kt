...
class ItemController(
    ...
    fun listItems(
        ...
        val items = when {
            categoryId != null -> itemRepository.findByCategoryIdAndStatus(categoryId, ItemStatus.ACTIVE)
            else -> itemRepository.findByStatus(ItemStatus.ACTIVE)
        }.let { list ->
    ...
    private fun Item.toMap(): Map<String, Any> {
        ...
        return mapOf(
            ...
            "thumbnail" to (itemImageRepository.findByItemIdOrderBySortOrder(id).firstOrNull()
                ?.let { "/api/v1/images/${it.path}" } ?: ""),
        )
    }
