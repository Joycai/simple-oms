...
class OrderService(
    ...
    fun checkout(buyerId: String): List<Order> {
        ...
        // Load all items in one pass to avoid N+1 queries
        val itemIds = cartItems.map { it.itemId }.distinct()
        val itemMap = itemRepository.findAllById(itemIds).associateBy { it.id }

        // Group cart items by seller using cached item map
        val bySeller = cartItems.groupBy { ci ->
            itemMap[ci.itemId]?.sellerId ?: throw IllegalStateException("Item ${ci.itemId} not found")
        }
        ...
        for ((sellerId, items) in bySeller) {
            ...
            for (ci in items) {
                ...
                if (updated == 0) {
                    throw IllegalStateException("Insufficient stock for item ${ci.itemId}")
                }
                // Re-read from DB after deductStock to get accurate post-deduct state
                val item = itemMap[ci.itemId]!!
