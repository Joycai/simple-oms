...
class CartController(
    ...
    fun updateQuantity(
        ...
        if (ci.buyerId != buyerId) return ResponseEntity.status(403).body(mapOf("message" to "Access denied"))
    ...
    fun removeItem(@PathVariable id: Long, @AuthenticationPrincipal buyerId: String): ResponseEntity<Map<String, String>> {
        val ci = cartItemRepository.findById(id).orElse(null)
            ?: return ResponseEntity.notFound().build()
        if (ci.buyerId != buyerId) return ResponseEntity.status(403).body(mapOf("message" to "Access denied"))
