...
class OrderController(
    ...
    fun getOrder(@PathVariable id: Long, @AuthenticationPrincipal callerId: String): ResponseEntity<Map<String, Any>> {
        val order = orderRepository.findById(id).orElse(null)
            ?: return ResponseEntity.notFound().build()
        if (order.buyerId != callerId && order.sellerId != callerId) {
            return ResponseEntity.status(403).body(mapOf("message" to "Access denied"))
        }
