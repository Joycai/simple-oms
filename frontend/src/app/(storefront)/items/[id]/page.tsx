...
export default function ItemDetailPage() {
  ...
  async function handleAddToCart() {
    if (!getToken()) { router.push('/login?redirect=' + encodeURIComponent(window.location.pathname)); return }
    setMsg('')
    try {
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <div className="aspect-[4/3] overflow-hidden rounded-2xl bg-slate-100 dark:bg-slate-800">
          {images[0] ? (
            ...
            <div className="flex h-full items-center justify-center text-6xl font-bold text-slate-300">{item.name?.substring(0, 2).toUpperCase()}</div>
          )}
        </div>
        <div>
          ...
          {item.quantity > 0 ? (
            <div className="mt-6 flex items-center gap-4">
              <div className="flex items-center rounded-lg border dark:border-slate-700">
                <button onClick={() => setQty(Math.max(1, qty - 1))} className="px-3 py-2 text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800">-</button>
                <span className="px-4 py-2 text-sm font-medium">{qty}</span>
                <button onClick={() => setQty(Math.min(item.quantity, qty + 1))} className="px-3 py-2 text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800">+</button>
              </div>
              <button onClick={handleAddToCart} className="rounded-lg bg-indigo-600 px-6 py-2 text-sm font-medium text-white hover:bg-indigo-700">
