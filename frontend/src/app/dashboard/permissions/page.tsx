  ...
  return (
    <header className="flex h-14 items-center justify-between border-b border-slate-200 bg-white px-6 dark:border-slate-800 dark:bg-slate-950">
      <div className="flex items-center gap-4 text-sm">
        ...
        <span className="text-slate-900 dark:text-slate-50">{t('permissions.title')}</span>
      </div>
...
export default function PermissionsPage() {
  ...
  function groupChecked(group: PermGroup): boolean {
    return group.permissions.every(p => assignedPerms.has(p.id))
  }
              ...
              {selectedRoleId !== null && (
                <div className="mt-6 space-y-4">
                        ...
                        {group.permissions.map(perm => (
                          <label key={perm.id} className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                            <input
                              ...
                              className="h-3.5 w-3.5 rounded border-slate-300 text-indigo-900"
                            />
                            {perm.name}
                          </label>
                        ))}
                  ...
                  <button onClick={savePermissions}
                    className="rounded-lg bg-indigo-900 px-6 py-2 text-sm font-medium text-white hover:bg-indigo-800">
