...
const navGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'settings', href: '/dashboard/settings', labelZh: '安全中心', labelEn: 'Security', icon: SettingsIcon },
    ],
  },
  {
    ...
    <aside className={`flex flex-col border-r border-slate-200 bg-white transition-all duration-300 dark:border-slate-800 dark:bg-slate-950 ${collapsed ? 'w-16' : 'w-64'}`}>
      ...
      <div className="border-t border-slate-200 px-3 py-3 dark:border-slate-800">
        ...
        <button onClick={handleLogout}
          className="mt-2 flex w-full items-center gap-2 rounded-lg px-3 py-1.5 text-xs text-slate-500 hover:bg-red-50 hover:text-red-600 dark:text-slate-400 dark:hover:bg-red-950 dark:hover:text-red-400"
    ...
    <div className="flex h-screen">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-auto bg-slate-50 p-6 dark:bg-slate-950">
          {isCurrentAdminPath && mounted && !isAdmin ? null : children}
        </main>
      </div>
    </div>
  )
