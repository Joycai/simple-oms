...
export default function OtpSetupPage() {
  ...
  async function load() {
    ...
    const [meRes, codesRes] = await Promise.all([
      apiFetch('/auth/user/me', { headers: h }),
      apiFetch('/auth/otp/recovery-codes', { headers: h }),
    ])
    if (meRes.ok) {
    ...
    const setupRes = await apiFetch('/auth/otp/setup', { headers: h })
  ...
  async function regenerate() {
    const token = getToken()
    const res = await apiFetch('/auth/otp/regenerate-codes', { method: 'POST', headers: { Authorization: `Bearer ${token}` } })
